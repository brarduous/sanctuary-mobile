...
export default function PaywallScreen() {
  const { packages, purchasePackage, isPro, restorePurchases, isLoaded, loadOfferings } = useRevenueCat();
  const router = useRouter();
  const [isPurchasing, setIsPurchasing] = useState(false);

  useEffect(() => {
    if (isPro) {
        Alert.alert("Success", "You have access to Sanctuary Pro!");
        router.back();
    }
  }, [isPro]);
  ...
  return (
    <SafeAreaView className="flex-1 bg-white dark:bg-slate-950 p-6">
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {!isLoaded ? (
            ...
            <View className="py-10 items-center">
                ...
                <TouchableOpacity 
                    ...
                    className="bg-slate-200 dark:bg-slate-800 px-6 py-2 rounded-full"
                >
        ...
        <TouchableOpacity onPress={restorePurchases} className="mt-8 mb-8">
